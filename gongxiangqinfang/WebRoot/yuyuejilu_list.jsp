			  ...
			  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="yuyuejiluList.do" name="myform" method="post">
				...
				%>
                            <tr align="center" bgcolor="#FFFFFF">
                             ...
                             <td>${u.bianhao}</td>    <td>${u.mingcheng}</td>    <td>${u.zujin}</td>    <td>${u.kaishiriqi}</td>    <td>${u.jieshuriqi}</td>    <td>${u.qinfangzhu}</td>    <td>${u.yuyueriqi}</td>    <td>${u.yuyueshiduan}</td>    <td>${u.yuyuezhuangtai}</td>        <td>${u.yuyueren}</td>    <td align='center'>${u.issh} <input type="button" value="审核" onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=yuyuejilu'"/></td>
                              ...
                              <td align="center"><a href="doUpdateYuyuejilu.do?id=${u.id }">编辑</a>  <a href="deleteYuyuejilu.do?id=${u.id }"
...
while(RS_result.next()){
		...
		共计租金:<%=zujinz%>； 
		 ...
		 <c:if test="${page.page>1}">
              <a href="yuyuejiluList.do?page=1" >首页</a>
             <a href="yuyuejiluList.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="yuyuejiluList.do?page=${page.page+1 }">下一页</a>
			<a href="yuyuejiluList.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
